...
//        Classroom cr = new ClassroomRes(c.getId(),c.getName());
//        data.add(cr);
//    }

        return data;
    }
    public void create(ClassroomReq req) throws Exception {
        if(req.getName().isEmpty() || req.getName().length() < 6) {
            throw new Exception("Please enter a valid name for the classroom");
